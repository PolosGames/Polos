...
namespace polos
{
	...
	template<class event_type, typename ...Args>
	inline void event_bus::raise_event(Args && ...args)
	{
	...
	template<class event_type, class object_type, void(object_type::* method_ptr)(event_type &)>
	inline void event_bus::subscribe_to_event(object_type * const ptr)
	{
	...
	template<class event_type>
	inline void event_bus::subscribe_to_event(const delegate<void(event_type&)>& cback)
	{
	...
	template<class event_type>
	inline void event_bus::subscribe_to_event(delegate<void(event_type&)>&& cback)
	{
		auto &cbs = instance()._callbacks;
		event_id id = event_type::id;
	...
	template<class event_type>
	inline void event_bus::unsubscribe_from_event(const delegate<void(event_type&)>& cback)
	{
		auto &cbs = instance()._callbacks;
		event_id id = event_type::id;
