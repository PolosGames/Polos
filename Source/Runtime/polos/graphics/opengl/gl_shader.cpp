...
void Shader::CreateUniformLookup()
{
    ...
    for (GLuint i = 0; i < uniform_count; ++i)
    {
        ...
        m_LookupTable.insert(
            {GetStringId(uniform_name.get()), {glGetUniformLocation(m_ProgramId, uniform_name.get()), count}});
    }
}
...
void Shader::SetUniform(StringId const p_Name, std::span<int32> p_Value) const
{
    ...
    switch (size)
    {
        case 2: glUniform2iv(m_LookupTable.at(p_Name).location, 2, p_Value.data()); break;
        case 3: glUniform3iv(m_LookupTable.at(p_Name).location, 3, p_Value.data()); break;
        case 4: glUniform4iv(m_LookupTable.at(p_Name).location, 4, p_Value.data()); break;
        default:
...
void Shader::SetUniform(StringId p_Name, std::span<uint32> p_Value) const
{
    ...
    switch (size)
    {
        case 2: glUniform2uiv(m_LookupTable.at(p_Name).location, 2, p_Value.data()); break;
        case 3: glUniform3uiv(m_LookupTable.at(p_Name).location, 3, p_Value.data()); break;
        case 4: glUniform4uiv(m_LookupTable.at(p_Name).location, 4, p_Value.data()); break;
        default:
...
void Shader::SetUniform(StringId const p_Name, std::span<float> p_Value) const
{
    ...
    switch (size)
    {
        case 2: glUniform2fv(m_LookupTable.at(p_Name).location, 2, data); break;
        case 3: glUniform3fv(m_LookupTable.at(p_Name).location, 3, data); break;
        case 4: glUniform4fv(m_LookupTable.at(p_Name).location, 4, data); break;
        default: LOG_ENGINE_WARN("The size of the array that's being passed to shader must be in range [2,4]."); break;
    }
}
// ======== ! FLOAT ! ========
...
void Shader::SetUniform(StringId const p_Name, std::span<double> p_Value) const
{
    ...
    switch (size)
    {
        case 2: glUniform2dv(m_LookupTable.at(p_Name).location, 2, data); break;
        case 3: glUniform3dv(m_LookupTable.at(p_Name).location, 3, data); break;
        case 4: glUniform4dv(m_LookupTable.at(p_Name).location, 4, data); break;
        default: LOG_ENGINE_WARN("The size of the array that's being passed to shader must be in range [2,4]."); break;
    }
}
// ======== ! DOUBLE ! ========
