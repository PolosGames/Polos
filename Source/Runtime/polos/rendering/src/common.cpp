...
auto RendererResourceHandle::Index() -> std::size_t
{
    return GetUnderlying<std::size_t>(id, kIndexShift, kIndexMask);
}

auto RendererResourceHandle::Index16() -> std::uint16_t
{
    return GetUnderlying<std::uint16_t>(id, kIndexShift, kIndexMask);
}

auto RendererResourceHandle::Type() -> RendererResourceType
{
    return GetUnderlying<RendererResourceType>(id, kTypeShift, kTypeMask);
}

auto RendererResourceHandle::Version() -> std::uint16_t
{
    return GetUnderlying<std::uint16_t>(id, kVersionShift, kVersionMask);
}

auto RendererResourceHandle::IsValid() -> bool
{
    return id != 0U;
}

auto RendererResourceHandle::Create(std::uint16_t t_index, RendererResourceType t_type, std::uint16_t t_version)
...
auto RendererResourceHandle::Invalid() -> RendererResourceHandle
{
    return {0U};
}
