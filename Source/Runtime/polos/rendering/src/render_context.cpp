...
VkCommandBuffer RenderContext::BeginSingleTimeCommands()
{
    VkCommandBufferAllocateInfo const alloc_info{
        .sType              = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
        .pNext              = nullptr,
        .commandPool        = s_render_context->m_command_pool,
        .level              = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
        .commandBufferCount = 1U,
    };
...
void RenderContext::EndSingleTimeCommands(VkCommandBuffer t_command_buffer)
{
    assert(VK_SUCCESS == vkEndCommandBuffer(t_command_buffer));
    ...
    VkSubmitInfo const submit_info{
        ...
        .commandBufferCount   = 1U,
        .pCommandBuffers      = &t_command_buffer,
