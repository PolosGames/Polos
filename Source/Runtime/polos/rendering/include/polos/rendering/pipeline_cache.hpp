...
public:
    ...
    auto Create(PipelineCacheCreateDetails const& t_details) -> Result<void>;
    ...
    auto GetPipeline(utils::string_id t_pipeline_name) const -> Result<VulkanPipeline>;

    /// @brief Gets a pipeline from the cache
    /// @param t_pipeline_name The name of the pipeline to get.
    auto GetPipeline(std::string_view t_pipeline_name) const -> Result<VulkanPipeline>;
    ...
    auto ConstructPipeline(GraphicsPipelineInfo const& t_pipeline_info) -> Result<VulkanPipeline>;
private:
    VkDevice m_device{VK_NULL_HANDLE};

    VulkanSwapchain* m_swapchain{nullptr};

    std::unordered_map<utils::string_id, VulkanPipeline> m_cache;
